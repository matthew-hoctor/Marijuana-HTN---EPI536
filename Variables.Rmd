...
# Cross tabulation of missing variables for T1, assuming missing smokers are kept
...
# Cross tabulation of missing variables for T1, assuming missing smokers are excluded

```{stata}
use "data\NHANES0718_new.dta", clear

#Missing Smoking Variable:
gen SMK_miss=0
  replace SMK_miss=1 if SMK_cat==.
  
gen include2=0
  replace include2=1 if include==1 & SMK_miss==0

ta EDUC_cat include2,m
ta gndr include2,m
ta SMK_cat include2,m
ta AL_cat include2,m

gen AGE_miss =0
  replace AGE_miss=1 if ridageyr==.
ta AGE_miss include2,m

gen BMI_miss=0
  replace BMI_miss=1 if bmxbmi==.
ta BMI_miss include2,m

gen PIR_miss=0
  replace PIR_miss=1 if indfmpir==.
ta PIR_miss include2,m

gen HEI_miss=0
  replace HEI_miss=1 if hei2015_total_score==.
ta HEI_miss include2,m
```
